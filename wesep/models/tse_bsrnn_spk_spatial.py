...
class TSE_NBC2_SPK_SPATIAL(nn.Module):
    def __init__(self, config):
        ...
        if self.full_input:
            self.sep_configs["spec_dim"] = 2 * len(self.spatial_configs['geometry']['mic_coords'])
            
        # Add spatial dims
        if self.spatial_configs["features"]["ipd"]["enabled"]:
            self.sep_configs["spec_dim"] += n_pairs * self.spatial_configs["features"]["ipd"]["num_encoder"]
        if self.spatial_configs["features"]["cdf"]["enabled"]:
            self.sep_configs["spec_dim"] += n_pairs * self.spatial_configs["features"]["cdf"]["num_encoder"]
        ...
        if self.spk_configs["features"]["usef"]["enabled"]:
            self.sep_configs["spec_dim"] += self.spk_configs["features"]["usef"]["emb_dim"] * 2
        ...
        if self.spk_configs["features"]["context"]["enabled"]:
            self.spk_configs["features"]["context"]["band"] = self.sep_configs["win"] // 2 + 1
    ...
    def forward(self, mix, cue):
        ...
        if self.spk_configs['features']['usef']['enabled']:
            ...
            spec_RI_single = torch.stack([spec_norm[:, 0].real, spec_norm[:, 0].imag], 1)
        ...
        if self.spk_configs['features']['tfmap']['enabled']:
            enroll_mag = torch.abs(self.sep_model.stft(wav_enroll)[-1])  
            enroll_tfmap = self.spk_ft.tfmap.compute(enroll_mag, torch.abs(spec_norm[:, 0])) 
            spec_feat = self.spk_ft.tfmap.post(spec_feat, enroll_tfmap.unsqueeze(1))
        ...
        if self.spk_configs['features']['context']['enabled']:
            ...
            encode_features = self.spk_ft.context.post(encode_features, enroll_context)
        ...
        if self.spk_configs['features']['spkemb']['enabled']:
            ...
            # Align embedding to dim_hidden for broadcasting over channels
            enroll_emb = enroll_emb.unsqueeze(2).unsqueeze(3)  # (B, F_e, 1, 1)
            encode_features = self.spk_ft.spkemb.post(encode_features, enroll_emb)
