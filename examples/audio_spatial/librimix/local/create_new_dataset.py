...
class DataGenerator0dB:

    def __init__(self, config_path, stage='test'):
        """
    ...
    def _get_files_per_speaker(self, sub_folder):
        ...
        for ext in extensions:
            all_files.extend(
                glob.glob(os.path.join(current_libri_path, '**', ext),
                          recursive=True))
    ...
    def _generate_room_config(self):
        r_cfg = self.cfg['room']
        dim_min, dim_max = r_cfg['dimensions']['min'], r_cfg['dimensions'][
            'max']
        ...
        # 移除了噪声的位置坐标生成
        return room_sz, rt60, mic_pos, mic_center, np.array(spk_pos)
    ...
    @staticmethod
    def _read_audio(file_path, target_len_samples):
    ...
    def run(self, count=3000):
        ...
        data_type = self.stage
        subsets = dataset_splits.get(data_type,
                                     ["test-clean"])  # 默认用 test-clean
        current_spk_map, current_spk_list = self._get_files_per_speaker(
            subsets)

        folder_name = "test_0dB_clean"  # 明确标识这是 0dB 无噪声测试集

        print(
            f"Generating {data_type} set (saving to '{folder_name}'): {count} utterances at strictly 0dB SIR..."
        )
        ...
        for idx in range(count):
            ...
            ov_min, ov_max = mix_cfg[
                'overlap_range'] if 'overlap_range' in mix_cfg else (0.8, 1.0)
            ...
            # --- 1. RIR ---
            room_sz, rt60, mic_pos, mic_center, spk_pos = self._generate_room_config(
            )

            beta = gpuRIR.beta_SabineEstimation(room_sz, rt60)
            nb_img = gpuRIR.t2n(rt60, room_sz)

            rir_speech = gpuRIR.simulateRIR(room_sz, beta, spk_pos, mic_pos,
                                            nb_img, rt60, self.sr)

            # --- 2. Simulation ---
            spk1_echoic_raw = signal.fftconvolve(src_audio[:, None],
                                                 rir_speech[0].T,
                                                 mode='full',
                                                 axes=0)
            spk2_echoic_raw = signal.fftconvolve(interf_audio[:, None],
                                                 rir_speech[1].T,
                                                 mode='full',
                                                 axes=0)
            ...
            # --- 4. 核心：强制 0dB 缩放 (Mixing & Scaling) ---
            target_power = np.mean(target_echoic**2) + 1e-12
            curr_interf_power = np.mean(interf_echoic**2) + 1e-12

            # 直接让干扰能量等于目标能量 (SIR = 0dB)
            interf_scale = np.sqrt(target_power / curr_interf_power)
            interf_echoic *= interf_scale

            mixture = target_echoic + interf_echoic

            # 防止削波 (Clipping prevention)
            max_amp = np.max(np.abs(mixture))
            if max_amp > 0.99:
                scale_down = 0.99 / max_amp
                mixture *= scale_down
                target_echoic *= scale_down
                interf_echoic *= scale_down

            # --- 5. 保存文件 ---
            sf.write(os.path.join(dirs['mix'], output_filename + '.wav'),
                     mixture, self.sr)
            sf.write(os.path.join(dirs['s1'], output_filename + '.wav'),
                     target_echoic, self.sr)
            sf.write(os.path.join(dirs['s2'], output_filename + '.wav'),
                     interf_echoic, self.sr)

            az1, el1 = self._calculate_angles(mic_center, spk_pos[0])
            az2, el2 = self._calculate_angles(mic_center, spk_pos[1])

            # 物理验证算一下，确保它是真正的 0.00 dB
            ...
            meta_data = {
                ...
                "sir_db": np.float32(real_sir),  # 这里应该严格等于 0
                "snr_db": np.float32(100.0),  # 无噪声，SNR无穷大，记为100
            ...
            np.save(os.path.join(dirs['spatial'], output_filename + '.npy'),
                    meta_data)

            if (idx + 1) % 100 == 0:
                print(
                    f"  [{data_type}] Generated {idx + 1} / {count} | Checked SIR: {real_sir:.2f} dB"
                )


if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description='Generate 0dB Ablation Dataset')
    ...
    # 强制生成 test 集，且生成数量由参数控制
    generator = DataGenerator0dB(args.config, stage='test')
